...
namespace Modules\Serikatpekerja\Http\Controllers;
...
class SerikatpekerjaController extends Controller
{
    ...
    {
        // dd(\App\Helper\CustomHelper::getUniq());
        return view('serikatpekerja::SerikatpekerjaIndexView');
    }
    ...
    public function api(){
        ...
        $model = SerikatpekerjaModel::getData($parm1, $parm2, $parm3) ;
        return Datatables::of($model)
                            ->addColumn('action', function ($model) {

                                if (Auth::user()->role_id == 'SP') 
                                {
                                    if ($model->status == 1 ) 
                                    {
                                        return '
                                        <div class="list-icons">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right">
                                                    <a href="'.route('serikatpekerja.general', Crypt::encrypt($model->serikat_pekerja_id) ).'" class="dropdown-item"><i class="icon-pencil5"></i>  Isi Data</a>

                                                    <a href="'.route('serikatpekerja.anggota', Crypt::encrypt($model->serikat_pekerja_id) ).'" class="dropdown-item"><i class="icon-users"></i> Anggota</a>

                                                    <a href="'.route('serikatpekerja.pengurus', Crypt::encrypt($model->serikat_pekerja_id) ).'" class="dropdown-item"><i class="icon-user-tie"></i>  Pengurus</a>
                                        ...
                                        <div class="list-icons">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right">
                                                    ...
                                                    <a href="'.route('serikatpekerja.detail', Crypt::encrypt($model->serikat_pekerja_id) ).'" class="dropdown-item"><i class="icon-file-text"></i>  Detail</a>
                                                    <a href="'.route('serikatpekerja.bukti', Crypt::encrypt($model->serikat_pekerja_id) ).'" class="dropdown-item" target="_blank"><i class="icon-printer"></i>  Bukti Pencatatan</a>
                                                </div>
                                            </div>
                                        </div>
                                        ';
                                        ...
                                        <div class="list-icons">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right">
                                                    ...
                                                    <a href="'.route('serikatpekerja.detail', Crypt::encrypt($model->serikat_pekerja_id) ).'" class="dropdown-item"><i class="icon-file-text"></i>  Detail</a>
                                                    <a href="'.route('serikatpekerja.suratPenangguhan', Crypt::encrypt($model->serikat_pekerja_id) ).'" class="dropdown-item" target="_blank"><i class="icon-printer"></i>  Surat Penangguhan</a>
                                                </div>
                                            </div>
                                        </div>
                                        ';
                                    ...
                                    if ($model->status == 3 ) 
                                    {
                                       ...
                                       return '
                                        ...
                                        ';
                                          // Riwayat ADRT
                                          // <a href="'.route('serikatpekerja.riwayatAdART', Crypt::encrypt($model->serikat_pekerja_id) ).'" class="dropdown-item"><i class="icon-file-text2"></i>  Riwayat AD ART</a>
                            ...
                            ->addColumn('kirim', function ($model) {
                                ...
                                if (Auth::user()->role_id == 'SP') {
                                    ...
                                    if ($model->status == 1) {
                                        ...
                                        return '<a href="'.route('serikatpekerja.kirim', Crypt::encrypt($model->serikat_pekerja_id) ).'" 
                                    ...
                                    if ($model->status != '1') {
                                        ...
                                        return '<a href="'.route('serikatpekerja.proses', Crypt::encrypt($model->serikat_pekerja_id) ).'" 
    ...
    {
        return view('serikatpekerja::SerikatpekerjaCreateView');
    }
    ...
    public function createSave(Request $request){
        $this->validate($request, [
            ...
            // 'url_ad' => 'required|mimes:pdf|max:2000',
            ...
            'url_logo' => 'required|mimes:jpg,png,jpeg,pdf|max:2000'
        ], 
        [
        ...
        $date = date('Y-m-d');
        // $uploadedFile = $request->file('url_ad');
        ...
        return redirect()->route('serikatpekerja.general', Crypt::encrypt($id_transaction));
    ...
    {   
        ...
        return view('serikatpekerja::SerikatpekerjaGeneralView', compact('model','ad'));
    ...
    //     return view('serikatpekerja::SerikatpekerjaEditView', compact('model'));

    // }

    // public function editSave(Request $request){
    //     $this->validate($request, [
    //         'serikat_pekerja_desc' => 'required|string|max:255|unique:tr_serikat_pekerja,serikat_pekerja_desc,'.$request->id,
    //         'id_provinsi' => 'required|string|max:255',
    //         'id_kabupaten' => 'required|string|max:255',
    //         'id_kecamatan' => 'required|string|max:255',
    //         'id_desa' => 'required|string|max:255',
    ...
    //         'url_ad' => 'mimes:jpg,png,jpeg,pdf|max:2000',
    //         'tgl_pembuatan_ad' => 'required|string|max:255',
    //         'tgl_berlaku_ad' => 'required|string|max:255',

    //         'nama_singkat' => 'string|max:255|unique:tr_serikat_pekerja,nama_singkat,'.$request->id,
    //         'url_logo' => 'required|mimes:jpg,png,jpeg,pdf|max:2000',
    //         'url_ad' => 'required|mimes:jpg,png,jpeg,pdf|max:2000',
    //     ],[
    //         'required' => 'Harus di isi.',
    //         'mimes' => 'Upload file dengan extention .jpg .jpeg .png .pdf',
    //         'unique' => 'Data sudah ada'
    //     ]);
    ...
    //     if ($request->url_ad == null ) {
    //         $path = $request->url_ad_hidden ;
    //     }else{
    //         $uploadedFile = $request->file('url_ad');
    //         $path = $uploadedFile->store('public/anggaran/'.$date);
    //     }
    ...
    //         $path_logo = $request->url_logo_hidden ;
    //     }else{
    //         $file_logo = $request->file('url_logo');
    //         $path_logo = $file_logo->store('public/logo/'.$date);   
    //     }
    ...
    //         $newUser = AnggaranDasarModel::updateOrCreate([
    ...
    //     $query = SerikatpekerjaModel::where('serikat_pekerja_id', $request->serikat_pekerja_id)->update($update_sp) ;
    ...
    {   
        $decrypted = Crypt::decrypt($id);     
        $model = SerikatpekerjaModel::where('serikat_pekerja_id', $decrypted)->first();
        return view('serikatpekerja::SerikatpekerjaAnggotaView', compact('model'));

    }

    public function anggotaSave(Request $request){
        $this->validate($request, [
            'anggota_nik' => 'required|string|max:16|min:16|unique:tr_anggota,anggota_nik',
            'anggota_nama' => 'required|string|max:200',
            'jk' => 'required|string|max:200'
            // 'url_kta' => 'mimes:jpg,png,jpeg,pdf|max:2000'

        ],[
            'required' => 'Harus diisi',
            'anggota_nik.max' => 'Harus 16 Angka',
            'anggota_nik.unique' => 'Data sudah ada',
            'mimes' => 'Upload file dengan extention .jpg .jpeg',
            'anggota_nik.min' => 'Harus 16 Angka'
        ]);


        // $date = date('Y-m-d');
        // $uploadedFile = $request->file('url_kta');
        // $path = $uploadedFile->store('public/kta/'.$date);   

        DB::beginTransaction();
        try {
            $query = AnggotaModel::insert([
                                    'serikat_pekerja_id' => $request->serikat_pekerja_id,
    ...
    public function apiAnggota(Request $request){

        $model = AnggotaModel::getData()->where('serikat_pekerja_id',$request->id) ;
        return Datatables::of($model)
                            ->addColumn('delete', function ($model) {
                                return '<a href="#"
                                        class="del_modal"
                                        data-id="'.$model->anggota_nik.'" 
                                        data-desc="'.$model->anggota_nama.'" 
                                        title="DELETE: '.$model->anggota_nama.'">
    ...
    public function destroyAnggota(Request $request)
    {
        ...
        $model = AnggotaModel::where('anggota_nik', $request->id)->delete();
    ...
    {   
        $decrypted = Crypt::decrypt($id);     
        $model = SerikatpekerjaModel::where('serikat_pekerja_id', $decrypted)->first();
        return view('serikatpekerja::SerikatpekerjaPengurusView', compact('model'));
    ...
    public function kirim($id)
    {
        $decrypted = Crypt::decrypt($id);     
        $model = SerikatpekerjaModel::where('serikat_pekerja_id', $decrypted)->first();
        return view('serikatpekerja::SerikatpekerjaKirimView', compact('model'));
    ...
    {   
        ...
        $decrypted = Crypt::decrypt($id);     
        $model = SerikatpekerjaModel::where('serikat_pekerja_id', $decrypted)->first();
        return view('serikatpekerja::SerikatpekerjaProsesView', compact('model'));
    ...
    public function prosesSave(Request $request)
    {
        ...
        $update_sp = array(
                        ...
                        'no_catat' => $no_catat,
                        'change_status_by' => Auth::user()->user_id,
                        'change_status_date' => date('Y-m-d H:i:s')   
                    );
    ...
    public function pengurusNext($id){
        return redirect()->route('serikatpekerja.pengurus', $id );
    }
    ...
    {   
        $decrypted = Crypt::decrypt($id);     
        $model = SerikatpekerjaModel::getDetail()->where('serikat_pekerja_id', $decrypted)->first();
        return view('serikatpekerja::SerikatpekerjaDetailView', compact('model'));

    }

    public function getProvinsi(Request $request)
    {   
        return CustomHelper::getProvinsi($request->id_provinsi, $request->idSelected );
    }

    public function getKabupaten(Request $request)
    {   
        return CustomHelper::getKabupaten($request->id_provinsi, $request->idSelected );
    }
    public function getKecamatan(Request $request)
    {   
        return CustomHelper::getKecamatan($request->id_kabupaten, $request->idSelected );
    }
    public function getDesa(Request $request)
    {   
        return CustomHelper::getDesa($request->id_kecamatan, $request->idSelected );
    }
    ...
    public function riwayatAdART($id)
    {   
        $decrypted = Crypt::decrypt($id);     
        $model = SerikatpekerjaModel::getDetail()->where('serikat_pekerja_id', $decrypted)->first();
        return view('serikatpekerja::SerikatpekerjariwayatAdARTView', compact('model'));

    }

    public function apiRiwayatAdART(Request $request){

        $model = AnggaranDasarModel::getData()->where('serikat_pekerja_id',$request->id) ;
        return Datatables::of($model)
                            ->addColumn('status', function ($model) {
                                if ($model->status == 1) {
                                    $status = '<span class="bg-danger px-2">Tidak Berlaku</span>' ;
                                } else {
                                    $status = '<span class="bg-success px-2">Berlaku</span>' ;
                                }
                                
                                return $status ;
                            })
                            ->addColumn('url_ad', function ($model) {
                                return '<a href="'. url('public' .Storage::url($model->url_ad)). '"
                                            target="_blank"
                                            title="Download AD ART: '.$model->keterangan.'">
                                            <i class="icon-file-check2"></i></a>';
                            })
                            ->rawColumns(['status','url_ad'])
                            ->addIndexColumn()
                            ->make(true);
