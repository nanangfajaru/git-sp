...
<div class="card">
    <div class="card-header header-elements-inline" data-toggle="collapse" href="#accordion-item-default2">
                 <h6 class="card-title"><i class="icon-users4 mr-2"></i> Anggota Konfederasi </h6>
    ...
    <div id="accordion-item-default2" class="collapse show" data-parent="#accordion-default">
        ...
        <form action="{{ route('konfederasi.memberSave') }}" method="post" enctype="multipart/form-data">
            {{csrf_field()}}    
            <div class="card-body">
                <div class="row"> 
                        <div class="col-md-12">
                ...
                <div class="row"> 
                        <div class="col-md-4">
                            <div class="form-group {{ $errors->has('konfederasi_member') ? 'has-error' : '' }}" >
                                <label> Federasi :</label>
                                    {!! Form::select('konfederasi_member', $konfederasi_member, null, ['class' => 'form-control select-search','placeholder' => '- Pilih -', 'required' => '']) !!}       
                                    <span class="text-danger">{{ $errors->first('konfederasi_member') }}</span>
                            </div>
                        </div>
                </div>
                <br>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-2">
                        {{-- <a href="{{ route('konfederasi.pengurusNext', Crypt::encrypt($model->serikat_pekerja_id)) }}" type="button" class="btn bg-{{ \Config::get('app.theme')}} pull-right" id="btnNext">Pengurus <i class="icon-square-right ml-2"></i></a> --}}
                        <a href="{{ route('konfederasi') }}" type="button" class="btn bg-{{ \Config::get('app.theme')}} pull-right" id="btnNext">Selesai <i class="icon-square-right ml-2"></i></a>
                    </div>
                </div>
            </div>
...
<script>
    $(document).ready(function() { 
        $('#TableId').DataTable({
            ...
            ajax: {
                ...
                url: "{{ route('konfederasi.apiMember') }}",
                type: "POST",
                data: {
    ...
    $('.modal-footer').on('click', '.btn_delete', function() {
        $.ajax({
            type: 'post',
            url: '{{ route('konfederasi.destroyMember')}}',
            data: {
